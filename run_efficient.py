#!/usr/bin/env python
"""
Efficient version of the TransformerNMT model training script.

This script uses only 20% of the data and reduced hyperparameters to allow for
faster training while still demonstrating the Transformer architecture.
"""
import os
import sys
import argparse
...
# Import functionality from run_model_full.py
...
class EfficientConfigParams:
    """Small configuration with reduced hyperparameters."""
    def __init__(self, base_params=None):
        ...
        # Override with smaller hyperparameters
        self.batch_size = 64  # Smaller batch size
        self.d_model = 256    # Smaller model dimension (was 512)
        self.n_layers = 3     # Fewer layers (was 6)
        self.n_heads = 4      # Fewer attention heads (was 8)
        self.ffn_hidden = 1024  # Smaller FFN dimension (was 2048)
        self.max_seq_length = 128  # Shorter sequences (was 256)
        self.epochs = 50      # Target number of epochs
        self.dropout = 0.1    # Keep dropout the same


class EfficientIWSLTDataset:
    """
    A more efficient version that uses only 20% of the data.
    """
    ...
    def __init__(
        ...
        batch_size,
        device,
        subset_ratio=0.2,  # Use 20% of the data
        max_length=128,
        min_freq=2
    ):
        """
        Initialize the dataset handler with subset sampling.
        ...
        Args:
            ...
            subset_ratio: Fraction of the data to use (0.2 = 20%)
    ...
    def _create_field(self, tokenize_fn, is_source):
        ...
        return ModernField(
            ...
            fix_length=self.max_length,  # Make sure to enforce max length
            use_vocab=True
        )
        
    def _load_data(self):
        """
        Load the manually downloaded IWSLT English-Vietnamese dataset and sample 20%.
        
        Returns:
            Tuple of (train, validation, test) datasets
        """
        print(f"Loading IWSLT English-Vietnamese dataset (using {self.subset_ratio*100:.0f}% of data)...")
        ...
        # Load all data
        train_src_full = read_file(train_en_path)
        train_tgt_full = read_file(train_vi_path)
        valid_src_full = read_file(valid_en_path)
        valid_tgt_full = read_file(valid_vi_path)
        test_src_full = read_file(test_en_path)
        test_tgt_full = read_file(test_vi_path)
        
        # Filter out very long sequences
        # This is to prevent dimension mismatch with positional encoding
        max_len = self.max_length - 2  # Allow space for SOS and EOS tokens
        
        def filter_pair(src, tgt):
            # Check if either source or target would exceed max length after tokenization
            src_tokens = self.tokenizer.tokenize_en(src)
            tgt_tokens = self.tokenizer.tokenize_vi(tgt)
            return len(src_tokens) <= max_len and len(tgt_tokens) <= max_len
        
        # Filter training data
        filtered_train_pairs = [(src, tgt) for src, tgt in zip(train_src_full, train_tgt_full) 
                                if filter_pair(src, tgt)]
        print(f"  Filtered {len(train_src_full) - len(filtered_train_pairs)} training pairs due to length")
        
        # Unzip filtered pairs
        if filtered_train_pairs:
            train_src_full, train_tgt_full = zip(*filtered_train_pairs)
        ...
        train_indices = random.sample(range(len(train_src_full)), 
                                      k=int(len(train_src_full) * self.subset_ratio))
        train_src = [train_src_full[i] for i in train_indices]
        train_tgt = [train_tgt_full[i] for i in train_indices]
        
        # Sample validation data - apply length filtering here too
        filtered_valid_pairs = [(src, tgt) for src, tgt in zip(valid_src_full, valid_tgt_full)
                                if filter_pair(src, tgt)]
        if filtered_valid_pairs:
            valid_src_full, valid_tgt_full = zip(*filtered_valid_pairs)
        
        valid_indices = random.sample(range(len(valid_src_full)), 
                                      k=min(int(len(valid_src_full) * self.subset_ratio), len(valid_src_full)))
        valid_src = [valid_src_full[i] for i in valid_indices]
        valid_tgt = [valid_tgt_full[i] for i in valid_indices]
        
        # Sample test data - apply length filtering here too
        filtered_test_pairs = [(src, tgt) for src, tgt in zip(test_src_full, test_tgt_full)
                              if filter_pair(src, tgt)]
        if filtered_test_pairs:
            test_src_full, test_tgt_full = zip(*filtered_test_pairs)
            
        test_indices = random.sample(range(len(test_src_full)), 
                                     k=min(int(len(test_src_full) * self.subset_ratio), len(test_src_full)))
        test_src = [test_src_full[i] for i in test_indices]
        test_tgt = [test_tgt_full[i] for i in test_indices]
        
        # Create datasets using our efficient dataset class
        train_data = EfficientTranslationDataset(
            train_src, train_tgt, 
            self.source_field, self.target_field,
            max_length=self.max_length
        )
        
        valid_data = EfficientTranslationDataset(
            valid_src, valid_tgt, 
            self.source_field, self.target_field,
            max_length=self.max_length
        )
        
        test_data = EfficientTranslationDataset(
            test_src, test_tgt, 
            self.source_field, self.target_field,
            max_length=self.max_length
        )
        ...
        # Print dataset sizes
        print(f"  Full Train: {len(train_src_full)} examples, Sampled: {len(train_data)} examples ({len(train_data)/len(train_src_full)*100:.1f}%)")
        print(f"  Full Validation: {len(valid_src_full)} examples, Sampled: {len(valid_data)} examples ({len(valid_data)/len(valid_src_full)*100:.1f}%)")
        print(f"  Full Test: {len(test_src_full)} examples, Sampled: {len(test_data)} examples ({len(test_data)/len(test_src_full)*100:.1f}%)")
...
class EfficientTranslationDataset(TranslationDataset):
    """
    Dataset for machine translation with source and target texts, with sequence length control.
    This extends the original TranslationDataset but ensures sequences are properly truncated.
    """
    def __init__(self, src_texts, tgt_texts, src_field, tgt_field, max_length=128):
        """
        ...
        """
        super().__init__(src_texts, tgt_texts, src_field, tgt_field)
        self.max_length = max_length
        
    def __getitem__(self, idx):
        """
        Get example at index, ensuring proper truncation.
        ...
        # Process using fields - this will tokenize and add special tokens
        src_processed = self.src_field.process(src_text)
        tgt_processed = self.tgt_field.process(tgt_text)
        
        # Ensure sequences are not longer than max_length
        if len(src_processed) > self.max_length:
            # For source, keep initial tokens and the EOS token if present
            src_processed = src_processed[:self.max_length-1] + [src_processed[-1]]
            
        if len(tgt_processed) > self.max_length:
            # For target, keep SOS token, some content tokens, and EOS token
            tgt_processed = [tgt_processed[0]] + tgt_processed[1:self.max_length-1] + [tgt_processed[-1]]
...
def setup_model_and_dataset(args, hyperparams):
    ...
    try:
        # Use our custom dataset loader with subset sampling
        dataset = EfficientIWSLTDataset(
            tokenizer=tokenizer,
...
def train(args, hyperparams):
    ...
    print("\nCalculating expected training time for efficient training...")
    batch_size = hyperparams.batch_size
    ...
    trainer = Trainer(
        ...
        log_dir="results/logs_efficient",
        save_dir="results/checkpoints_efficient"
    )
...
def main():
    """
    Main function for running the efficient TransformerNMT model.
    """
    # Set up command line arguments
    parser = argparse.ArgumentParser(description="Run efficient TransformerNMT model with reduced dataset")
    ...
    # Create efficient hyperparameters based on the original ones
    hyperparams = EfficientConfigParams(params)
    
    # Check if we need to create directories
    if args.action == "train":
        os.makedirs("results/logs_efficient", exist_ok=True)
        os.makedirs("results/checkpoints_efficient", exist_ok=True)
